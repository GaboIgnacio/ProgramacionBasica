# Gabriel Melo 
# Sección: 1 (Ingeniería en Informatica y Ciberseguridad) IpLosLeones
...
# Si no cumple con el formato, mostramos un mensaje de error y el bucle vuelve a pedir la MAC
        print("❌ Dirección MAC inválida. Asegúrate de escribirla en el formato AA:BB:CC:DD:EE:FF con valores hexadecimales.")
...
def permitido_denegado():
    ...
    while True: # Iniciamos un bucle infinito, que se repetirá hasta que el usuario ingrese una opción válida
        ...
        if valor in opciones:  # Verificamos si la respuesta está en la lista de opciones válidas
            return valor.capitalize()  # Devolvemos la respuesta en formato capitalize: esto hace que la palabra quede con la primera letra en mayúscula y el resto en minúscula ("Permitido" o "Denegado")
...
# Mostramos los encabezados de las columnas, cada uno con un ancho fijo para que queden alineados visualmente como una tabla
    ...
    print("=" * 100)  # Línea separadora decorativa
# f es una f-string, que permite insertar variables o textos con formato dentro de una cadena
...
def agregar():
    ...
    # Guardamos todos los datos en una lista interna llamada datos
...
# Comprobamos la opción seleccionada por el usuario y ejecutamos la función correspondiente
        if opcion == "1":  # Si la opción es '1', llamamos a la función para agregar un nuevo dato
            ...
            eliminar()
        elif opcion == "3":  # Si la opción es '3', llamamos a la función para agregar en una ubicación específica
            agregar_en_posicion()
        elif opcion == "4":  # Si la opción es '4', llamamos a la función para mostrar los datos ingresados
            mostrar_datos()
        elif opcion == "terminar":  # Si escribe 'terminar', salimos del codigo
            print("\n")
...
# En resumen Este código simula un sistema básico de gestión de conexiones de red, donde se almacenan datos como IPs, MACs, puertos y protocolos en una estructura de lista de listas (tipo matriz).
# Se utilizan validaciones con expresiones regulares para asegurar que los datos ingresados (como direcciones IP y MAC) sean válidos.
# Además, se proporciona una función para mostrar los datos de forma ordenada y clara en pantalla, con formato de tabla.
# Todo esto permite registrar, validar y mostrar información de tráfico de red de manera sencilla.
